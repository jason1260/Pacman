...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M, Pacman* pman) {
	...
	for (Directions i = 1; i <= 4; i++)
		if (ghost_movable(ghost, M, i, true) && i != oppo) proba[cnt++] = i;
	if (!cnt) proba[cnt++] = oppo;
	if (ghost->typeFlag == Blinky) 
		ghost_NextMove(ghost, shortest_path_direc(M, ghost->objData.Coord.x,
			ghost->objData.Coord.y, pman->objData.Coord.x, pman->objData.Coord.y));
	else
