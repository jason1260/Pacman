...
static void draw_block_index(Map* M, int row, int col, int color);
static void draw_key(Map* M, int row, int col);
	...
	*/
	for (int row = 0; row < M->row_num; row++) {
		for (int col = 0; col < M->col_num; col++) {
			...
			{
				case '#':
					draw_block_index(M, row, col, gray);
					break;
				case '&':
					draw_block_index(M, row, col, yellow);
					break;
				case '%':
					draw_block_index(M, row, col, blue);
					break;
				case '@':
					draw_block_index(M, row, col, purple);
					break;
				case 'A':
					draw_key(M, row, col, purple);
					break;
				case '!':
					draw_key(M, row, col, blue);
					break;
				case 'P':
...
static void draw_block_index(Map* M, const int row, const int col, int color) {
	...
	{
	case 0:
		cl = al_map_rgb(120, 120, 120);
		break;
	case 1:
		cl = al_map_rgb(235, 37, 0);
		break;
	case 2:
		cl = al_map_rgb(235, 200, 0);
		break;
	case 3:
		cl = al_map_rgb(234, 117, 255);
		break;
	case 4:
		cl = al_map_rgb(27, 117, 255);
		break;
	default:
		cl = al_map_rgb(25, 154, 25);
		break;
	}
...
static void draw_key(Map* M, const int row, const int col, const int color) {
	ALLEGRO_COLOR cl;
	switch (color)
	{
	case 0:
	...
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, cl);
}
...
bool is_wall_block(Map* M, int index_x, int index_y) {
	if (index_x < 0 || index_x >= M->col_num || index_y < 0 || index_y >= M->row_num)
		return true;
	return (M->map[index_y][index_x] == '#' || M->map[index_y][index_x] == '@' || M->map[index_y][index_x] == '&'
		|| M->map[index_y][index_x] == '%');
}
...
/*bool is_room_point(Map* M, int index_x, int index_y) {
